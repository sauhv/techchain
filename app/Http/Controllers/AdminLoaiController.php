...
class AdminLoaiController extends Controller
{
    ...
    public function edit( Request $request , string $id) {
        $loai = DB::table('loai')->where('id_loai', $id)->first(); //first lấy 1 record //get lấy tất cả record
        if ($loai==null){
    ...
    public function update(Request $request, string $id) {
        ...
        $request->session()->flash('thongbao', 'Đã cập nhập loại có tên ' . $ten_loai);
        return redirect('/admin/loai');
    }
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        $request->session()->flash('thongbao', 'Đã xóa loại'); //flash chuyển trang 
        return redirect('/admin/loai');
    }
}
